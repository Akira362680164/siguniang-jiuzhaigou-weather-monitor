...
class PipelineUnitTests(unittest.TestCase):
    ...
    def test_native_three_hour_accumulation_is_summed_once(self):
        times = [f"2026-09-22T{hour:02d}:00" for hour in (0, 3, 6, 9)]
        hourly = {"time": times}
        for suffix in ("", "_member01"):
            hourly[f"temperature_2m{suffix}"] = [1] * 4
            hourly[f"precipitation{suffix}"] = [0.8, 0.0, 0.0, 0.0]
            hourly[f"snowfall{suffix}"] = [0] * 4
        day = pipeline.member_daily_distributions(hourly, 2)[0]
